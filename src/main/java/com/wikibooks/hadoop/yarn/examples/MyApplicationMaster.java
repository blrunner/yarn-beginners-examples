...
/**
 * An ApplicationMaster for executing the HelloYarn on a set of launched
 ...
 *
 * <p>
 * The MyApplicationMaster is started on a container by the
 * <code>ResourceManager</code>'s launcher. The first thing that the
 * <code>MyApplicationMaster</code> needs to do is to connect and register itself
 * with the <code>ResourceManager</code>. The registration sets up information
 * within the <code>ResourceManager</code> regarding what host:port the
 * MyApplicationMaster is listening on to provide any form of functionality to a
 * client as well as a tracking url that a client can use to keep track of
 * status/job history if needed. However, in the HelloYarn, trackingurl
 * and appMasterHost:appMasterRpcPort are not supported.
 * </p>
 *
 * <p>
 * The <code>MyApplicationMaster</code> needs to send a heartbeat to the
 ...
 * <code>MyApplicationMaster</code> acts as a heartbeat.
 *
 * <p>
 * For the actual handling of the job, the <code>MyApplicationMaster</code> has to
 ...
 * <code>MyApplicationMaster</code> of the set of newly allocated containers,
 * completed containers as well as current state of available resources.
 * </p>
 *
 * <p>
 * For each allocated container, the <code>MyApplicationMaster</code> can then set
 ...
 *
 * <p>
 * The <code>MyApplicationMaster</code> can monitor the launched container by
 ...
 *
 * <p>
 * After the job has been completed, the <code>MyApplicationMaster</code> has to
 ...
 * <code>MyApplicationMaster</code> has been completed.
 */
...
public class MyApplicationMaster {
  ...
  // No. of containers to run the HelloYarn on
  private int numTotalContainers = 1;

  // Memory to request for the container on which the the HelloYarn will run
  private int containerMemory = 10;

  // VirtualCores to request for the container on which the the HelloYarn will run
  ...
  // Location of the HelloYarn jar file path in fs
  ...
  public static void main(String[] args) {
    ...
    if (result) {
      LOG.info("MyApplicationMaster completed successfully. exiting");
      System.exit(0);
    } else {
      LOG.info("MyApplicationMaster failed. exiting");
      System.exit(2);
    }
  }
  ...
  public boolean init(String[] args) throws ParseException, IOException {
    ...
    if (fileExist(log4jPath)) {
      try {
        Log4jPropertyHelper.updateLog4jConfiguration(MyApplicationMaster.class,
            log4jPath);
      } catch (Exception e) {
    ...
    if (envs.containsKey(Constants.AM_JAR_PATH)) {
      ...
      if (!appJarPath.isEmpty() && (appJarTimestamp <= 0 || appJarPathLen <= 0)) {
        LOG.error("Illegal values in env for the HelloYarn jar file path" + ", path="
            + appJarPath + ", len=" + appJarPathLen + ", timestamp="+ appJarTimestamp);
        throw new IllegalArgumentException(
            "Illegal values in env for the HelloYarn jar file path");
      }
    }
    ...
    if (numTotalContainers == 0) {
      throw new IllegalArgumentException("Cannot run the HelloYarn with no containers");
    }
  ...
  public void run() throws YarnException, IOException {
    ...
    // containers
    // Keep looping until all the containers are launched and the HelloYarn
  ...
  private AMRMClient.ContainerRequest setupContainerAskForRM() {
    ...
    // using * as any host will do for the HelloYarn
   ...
   * that will execute the HelloYarn.
   */
  private class LaunchContainerRunnable implements Runnable {
     ...
     * for the HelloYarn and eventually dispatches the container
    ...
    public void run() {
      ...
      ctx.setLocalResources(Collections.singletonMap(Constants.AM_JAR_NAME, appMasterJar));
      ...
      vargs.add("-Xmx256M");
      vargs.add("com.wikibooks.hadoop.yarn.examples.HelloYarn");
      ...
      // the container in the HelloYarn doesn't need any tokens. We are
      // populating them mainly for NodeManagers to be able to download any
      // files in the distributed file-system. The tokens are otherwise also
      // useful in cases, for e.g., when one is running a "hadoop dfs" command
      // inside the HelloYarn.
